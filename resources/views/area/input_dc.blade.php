                ...
                <div class="col-6">
                    <div class="card card-primary">
                        ...
                        @if (session()->has('success2'))
                            <div class="alert alert-success alert-dismissible text-center fade show" role="alert">
                                {{ session('success2') }}</div>
                        @endif
                        <div class="card-header">
                            <h3 class="card-title">Input Rayon DC</h3>
                        </div>
                        <!-- /.card-header -->
                        <div class="card-body">
                            <form action="/store-area-dc" method="POST">
                                @csrf
                                <div class="row">
                                    <div class="col-sm-8">
                                        <!-- select -->
                                        <div class="form-group">
                                            <label>CABANG</label>
                                            <select name="CODE_STOF" class="form-control">
                                                @foreach ($code_stof as $item)
                                                    <option value="{{ $item->CODE_STOF }}">{{ $item->SATELITE_OFFICE }}
                                                    </option>
                                                @endforeach
                                            </select>
                                        </div>
                                ...
                                <div class="card-footer">
                                    <button type="submit" name="create_area_dc" class="btn btn-info"><i
                                            class="fa-solid fa-send"></i>Save</button>
                                </div>
                            </form>
                        </div>
